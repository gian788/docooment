   ...
   self.server.queryDatabases('SELECT * FROM root r').toArray(function (err, databases) {
      ...
      self.server.queryCollections(self.database._self, 'SELECT * FROM root r').toArray(function (err, collections) {
         ...
         for(var i = 0; i < collections.length; i++){
            //self.collections[collections[i].id] = collections[i];
            self.collections[collections[i].id] = new Collection(collections[i], self);//, options
         }

         removeQueryUDF(self, function(err, delres){
            if(err) return callback(err);

            createQueryUDF(self, function(err, udfs){
               if (err) {
                  self.readyState = STATES.disconnected;
                  if (self._hasOpened) {
                     if (callback) callback(err);
...
function asyncMapFn(array, eachCb, callback){
   var results = [],
       count = 0,
       len = 0,
       error;
...
function removeQueryUDF(self, callback){
   asyncMapFn(
      self.collections,
      function(collection, cbColl){
         self.server.queryUserDefinedFunctions(collection.dbCollection._self, 'SELECT * FROM root r').toArray(function (err, udfs){
            if(err) return cbColl(err);
            
            asyncMapFn(
               udfs,
               function(udf, cbUdf){
                  self.server.deleteUserDefinedFunction(udf._self, cbUdf);   
               }, cbColl);
         });   
      }, callback);
}
   ...
   asyncMapFn(
      ...
      function(collection, cbColl){
         self.server.queryUserDefinedFunctions(collection.dbCollection._self, 'SELECT * FROM root r').toArray(function (err, udfs){
            ...
            asyncMapFn(
               newUDF,
               function(udfName, cbUdf){
                  self.server.createUserDefinedFunction(collection.dbCollection._self, {id: udfName, serverScript: queryUDF[udfName]}, cbUdf);   
               },cbColl);
         });   
      }, callback);
}
