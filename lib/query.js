...
mquery.Collection.prototype.find = function (match, options, cb) {
...
function Query(conditions, options, model, collection) {
  ...
  if (!this._docoomentOptions) {
    this._docoomentOptions = {};
  }
  ...
  if (options) {
    ...
    for (var i=0; i < keys.length; i++) {
      var k = keys[i];
      this._docoomentOptions[k] = options[k];
    }
  }

  if (collection) {
    this.docoomentCollection = collection;
  }
 ...
 *
 *     // Adventure is now a subclass of docooment.Query and works the same way but with the
...
Query.prototype.toConstructor = function toConstructor () {
  ...
  p.model = this.model;
  p.docoomentCollection = this.docoomentCollection;
  p._docoomentOptions = this._docoomentOptions;
 ...
 * ####NOTE:
 *
 * As of Docooment 3.7, `$geoWithin` is always used for queries. To change this behavior, see [Query.use$geoWithin](#query_Query-use%2524geoWithin).
 *
 * ####NOTE:
 *
 * In Docooment 3.7, `within` changed from a getter to a function. If you need the old syntax, use [this](https://github.com/ebensing/docooment-within).
 *
 * @method within
...
Query.prototype.setOptions = function (options, overwrite) {
  ...
  if (overwrite) {
    // ensure that _docoomentOptions & options are two different objects
    this._docoomentOptions = (options && utils.clone(options)) || {};
    this.options = options || {};

    if('populate' in options) {
      this.populate(this._docoomentOptions);
    }
    return this;
  }
 ...
 *
 * Documents returned from queries with the `lean` option enabled are plain javascript objects, not [DocoomentDocuments](#document-js). They have no `save` method, getters/setters or other Docooment magic applied.
 *
 * ####Example:
 *
 *     new Query().lean() // true
 ...
 *       docs[0] instanceof docooment.Document // false
 *     });
 *
 * This is a [great](https://groups.google.com/forum/#!topic/docooment-orm/u2_DzDydcnA/discussion) option in high-performance read-only scenarios, especially when combined with [stream](#query_Query-stream).
 *
...
Query.prototype.lean = function (v) {
  this._docoomentOptions.lean = arguments.length ? !!v : true;
  return this;
}
...
Query.prototype.find = function (conditions, callback) {
  ...
  var options = this._docoomentOptions;
  var self = this;
...
/**
 * Override mquery.prototype._mergeUpdate to handle docooment objects in
 * updates.
 *
 * @param {Object} doc
 * @api private
...
Query.prototype.update = function (conditions, doc, options, callback) {
  ...
  if (options) {
    if ('strict' in options) {
      this._docoomentOptions.strict = options.strict;
    }
  }
...
Query.prototype._walkUpdatePath = function _walkUpdatePath (obj, op, pref) {
  ...
  var strict = 'strict' in this._docoomentOptions
    ? this._docoomentOptions.strict
...
Query.prototype.populate = function () {
  ...
  var opts = this._docoomentOptions;
...
Query.prototype.cast = function (model, obj) {
  ...
  while (i--) {
    ...
    if ('$or' === path || '$nor' === path || '$and' === path) {
      ...
      while (k--) {
        orComponentQuery = new Query(val[k], {}, null, this.docoomentCollection);
      ...
      if (!schematype) {
        ...
        if (schematype) {
          ...
          if (schematype.caster && schematype.caster.schema) {
            ...
            castingQuery = new Query(remainingConds, {}, null, this.docoomentCollection);
 ...
 *
 * @param {Object} fields
 * @see https://github.com/LearnBoost/docooment/issues/1091
 ...
 *       // do something with the docooment document
 ...
 * ####Valid options
 *
 *   - `transform`: optional function which accepts a docooment document. The return value of the function will be emitted on `data`.
 *
 * ####Example
 *
...
// the rest of these are basically to support older Docooment syntax with mquery
 ...
 * ####NOTE:
 *
 * In Docooment 3.7, `intersects` changed from a getter to a function. If you need the old syntax, use [this](https://github.com/ebensing/docooment-within).
 *
 * @method intersects
 * @memberOf Query
 * @param {Object} [arg]
...
Query.prototype.near = function () {
  var params = [];
  var sphere = this._docoomentOptions.nearSphere;
...
Query.prototype.nearSphere = function () {
  this._docoomentOptions.nearSphere = true;
...
/*!
 * this is needed to support the docooment syntax of:
