 ...
 *
 * _NOTE: violating the constraint returns an `E11000` error from MongoDB when saving, not a Docooment validation error._
 *
 * @param {Boolean} bool
 * @return {SchemaType} this
 * @api public
 ...
 *
 * You can set up email lower case normalization easily via a Docooment setter.
 *
 *     function toLower (v) {
 ...
 *
 * Suppose you are storing credit card numbers and you want to hide everything except the last 4 digits to the docooment user. You can do so by defining a getter in the following way:
 *
 ...
 *
 * The error message argument is optional. If not passed, the [default generic error message template](#error_messages_DocoomentError-messages) will be used.
 *
 * ####Examples:
 *
 ...
 *
 * From the examples above, you may have noticed that error messages support baseic templating. There are a few other template keywords besides `{PATH}` and `{VALUE}` too. To find out more, details are available [here](#error_messages_DocoomentError-messages)
 *
 * ####Asynchronous validation:
 *
 * Passing a validator function that receives two arguments tells docooment that the validator is an asynchronous validator. The first argument passed to the validator function is the value being validated. The second argument is a callback function that must called when you finish validating the value and passed either `true` or `false` to communicate either success or failure respectively.
 *
  ...
  while (i--) {
    ...
    if (!(arg && 'Object' == arg.constructor.name)) {
      var msg = 'Invalid validator. Received (' + typeof arg + ') '
        + arg
        + '. See http://docoomentjs.com/docs/api.html#schematype_SchemaType-validate';
 ...
 * @return {SchemaType} this
 * @see Customized Error Messages #error_messages_DocoomentError-messages
 * @api public
 */
