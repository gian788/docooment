...
var EventEmitter = require('events').EventEmitter
  ...
  , ElemMatchError = DocoomentError.ElemMatchError
  , ValidationError = DocoomentError.ValidationError
 ...
 * @event `init`: Emitted on a document after it has was retreived from the db and fully hydrated by Docooment.
...
Document.prototype.$__set = function (
  ...
  if (shouldModify) {
    ...
    if (val instanceof DocoomentArray) {
...
Document.prototype.$__dirty = function () {
  ...
  all.forEach(function (item, i) {
    if (item.path.indexOf(lastPath) !== 0) {
      ...
      // special case for top level DocoomentArrays
 ...
 *
 * - `doc` The docooment document which is being converted
